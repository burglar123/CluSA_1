...
parser.add_argument("--dota_k", type = int, help = "dota_k", dest = "dota_k", required = False, default = 0)
...
model_config["dota_k"] =args.dota_k
...
print(model)
print(f"{task}_{attn_type}_lowrank_{uselowrank}_kk-{dota_k}_{sw}_{sigmoidrmv}")
...
if uselowrank == 1:
    log_f_path = os.path.join(checkpoint_dir, f"{task}_{attn_type}_lowrank_kk-{dota_k}_{sw}_{sigmoidrmv}_{timestamp}_output.log")
else:
    log_f_path = os.path.join(checkpoint_dir, f"{task}_{attn_type}__{sigmoidrmv}_{timestamp}_output.log")
