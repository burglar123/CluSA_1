...
class SBMAttention(nn.Module):
    def __init__(self, config):
        ...
        self.clusterskk = nn.Parameter(torch.empty(self.num_head, self.num_clusters, self.kk_dim))
        ...
        nn.init.kaiming_normal_(self.clusterskk)

    # def forward(self, Q, K, V, mask):
    def forward(self, Q, K, V, mask, QH, KH):
        ...
        dist = torch.matmul(self.clusterskk, torch.transpose(self.clusterskk, -1, -2)) 
        ...
        Qhatkk = nn.Sigmoid()(torch.matmul(QH, self.clusterskk.transpose(-1, -2)))
        Khatkk = nn.Sigmoid()(torch.matmul(KH, self.clusterskk.transpose(-1, -2)))

        # Qhatkk = torch.matmul(QH, self.clusterskk.transpose(-1, -2))
        # Khatkk = torch.matmul(KH, self.clusterskk.transpose(-1, -2))
        ...
        if self.type == 'fastRG':
            ...
            src, dst = fastRG(block_diag(Qhatkk.view(b*h,n,k)), 
                              block_diag(S.view(b*h,k,k)), 
                              block_diag(Khatkk.view(b*h,n,k)))
            ...
            eprobs = DF.u_dot_v(graph, 
                                Qhatkk.reshape(b*h*n, k), 
                                torch.matmul(Khatkk, S.transpose(-1,-2)).reshape(b*h*n,k))
            ...
            expA = torch.matmul(Qhatkk, torch.matmul(S, Khatkk.transpose(-1, -2)))
